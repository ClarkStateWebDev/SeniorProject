...
const InfectNotesForm = () => {
    ...
    return (
      <div style={{ textAlign: 'center' }}>
        <Button color="info" class="btn btn-warning" onClick={() => setShow(!show)} style={{float: 'right', marginRight: '32px', background: '#F5DE36', color: 'black'}}>
          Add/Edit Infection Notes
        </Button>
        <Modal
          show={show}
          toggle={() => setShow(!show)}
          title="Infection Notes Form"
          body={
            <div>
            <h7>Infection Notes</h7>
              <TextField
                name="test_field" defaultValue="enter infection notes" rows={5}
              />
  ...
  export default InfectNotesForm;